...
function addService() {
    return fetch("https://cdn.contentful.com/spaces/zwwrf2atjupp/environments/master/entries?access_token=RHpibft3v8j78VzU0mWayeySpv6_zaORfJGSe_MLiuM&&content_type=portfolioP")
        .then((res) => {
            return res.json()
        })
        .then((data) => {
            ...
            const fieldsColletions = data.items.map((item) => {
                const imag = searchImg(item.fields.img.sys.id, data);
                return {
                    titulo: item.fields.title,
                    paragraph: item.fields.explication,
...
function getWelcom(params = {}) {
    ...
    const c = document.querySelector('.contenedor-template-w');
...
function addW() {
    return fetch('https://cdn.contentful.com/spaces/zwwrf2atjupp/environments/master/entries?access_token=RHpibft3v8j78VzU0mWayeySpv6_zaORfJGSe_MLiuM&&content_type=portfolioW')
        ...
        .then((dato) => {
            ...
            const i = dato.includes.Asset[2].fields.file.url;
            return {
...
function main() {
    ...
    const welEl = document.querySelector('.contenedor-template-w');
    welcomeCardEl(welEl)


    const cardEle = document.querySelector('.container-servicios');
    cardEl(cardEle);
